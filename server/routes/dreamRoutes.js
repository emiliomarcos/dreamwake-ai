...
router.route("/").post(async (req, res) => {
  try {
    const { keywords, mainOutput, imageUrl, bulletsOutput, userId } = req.body;
    const image = await cloudinary.uploader.upload(`data:image/png;base64,${imageUrl}`, {folder: "dreamwake-ai"});

    const newDream = await Dream.create({keywords, mainOutput, imageUrl: image.url, bulletsOutput, userId});
